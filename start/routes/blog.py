...
async def get_blog_by_id(
    ...
    is_valid_auth = auth_svc.check_valid_auth(
        session_user, blog_author_id=blog.author_id, blog_email=blog.email
    )
    # 마지막에 본 게시글 세션에 저장하기
    if session_user:
...
async def create_blog_ui(
    request: Request, session_user=Depends(auth_svc.get_session_user_prt)
):
...
async def update_blog(
    ...
    if len(imagefile.filename.strip()) > 0:
        ...
        await blog_svc.update_blog(
            conn=conn, id=id, title=title, content=content, image_loc=image_loc
        )
    else:
        await blog_svc.update_blog(
            conn=conn, id=id, title=title, content=content, image_loc=image_loc
